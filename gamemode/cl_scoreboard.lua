	...
	for id,pl in pairs( player.GetAll() ) do
		...
		if (not TeamInfo[_team]) then
			...
			if GetGlobalInt("buildmode") == 0 then
				TeamInfo[_team].Color = Color(255,50,50,255)--team.GetColor( _team )
			else
				TeamInfo[_team].Color = Color(50,50,255,255)--team.GetColor( _team )
			end
			if pl:Alive() then
		...
		PlayerInfo.EXP = pl:GetNWInt("exp")
...
function GM:HUDDrawScoreBoard()
	...
	surface.SetDrawColor( 255, 255, 255, 20 )
	surface.DrawRect( xOffset + boardWidth - (colWidth*5.5), y, colWidth*1.5, boardHeight-y+yOffset )
	...
	surface.SetTextPos( xOffset + boardWidth - (colWidth*5.5) + 8,	y)	surface.DrawText("Most Time Alive")
	surface.SetTextPos( xOffset + boardWidth - (colWidth*4) + 8,	y)	surface.DrawText("#Deaths")
	surface.SetTextPos( xOffset + boardWidth - (colWidth*3) + 8,	y)	surface.DrawText("Money")
	surface.SetTextPos( xOffset + boardWidth - (colWidth*2) + 8,	y)	surface.DrawText("EXP")
	...
	for team,info in pairs(ScoreboardInfo) do
		...
		for index,plinfo in pairs(info.Players) do
			...
			draw.SimpleText( plinfo.EXP, "ScoreboardText", px+1, py+1, shadowcolor )
			draw.SimpleText( plinfo.EXP, "ScoreboardText", px, py, textcolor )
